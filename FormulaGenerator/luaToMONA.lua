...
local function generateHeader(auto)
    ...
    for k,v in pairs(automate.transitions) do
        if (table.contains(v, auto)) then
            return "pred "..k.."(var1 x, var2 "..declinerVariables("A","B","Pre","Post").. ",P, AX, Mot)=\n"
        end
    end
...
local function choixTransition()
    local res ='pred transitions(var2 '..declinerVariables('Pre','Post')..',P,AX,Mot)=\n'
    res = res .. 'ex2 ' .. declinerVariables('A','B')..' :((\n'
            ..'(all1 x :\n'
            ..'x in Mot => (\n'
    for k,_ in pairs(automate.transitions) do
        res = res ..k.."(x,"..declinerVariables("A","B","Pre","Post").. ",P, AX, Mot) |\n"
    end
    res = res:sub(1,-3)
    res = res .. ')) & \n'
